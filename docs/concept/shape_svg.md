...
- 矢量图元素集 Map<string, SVGElement>

    由自身直接维护的矢量图元素，被矢量图引用。

- 关键点集合 Set\<CriticalPoint\>
...
- 主从约束集 Map\<CriticalPoint, FollowingFunction\>
...
- 关键点初始化
    ...
    一个javascript脚本或json对象，结果是关键点对象数组。脚本中可以引用一些预定义内容，包括坐标系、预置活动域约束、预置主从约束。

- 路径点生成脚本

    一个javascript脚本，其中结果是返回一系列的函数。每个返回的函数的返回结果是一个json形式的路径点对象。脚本中可以引用一些预定义内容，包括坐标系、关键点定位、预置点函数等。
