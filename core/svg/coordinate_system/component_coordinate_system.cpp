...
namespace Lewzen {
    ...
    bool ComponentCoordinateSystem::operator== (const CoordinateSystem &coordianate_system) const {
        if (!A::operator==(coordianate)) return false;
        return _component == static_cast<const ComponentCoordinateSystem &>(coordianate).get_component();
    }
    bool ComponentCoordinateSystem::operator!= (const CoordinateSystem &coordianate_system) const {
        if (A::operator==(coordianate)) return false;
        return !_component == static_cast<const ComponentCoordinateSystem &>(coordianate).get_component();
