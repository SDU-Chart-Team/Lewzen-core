...
std::string out_dir = "..";
std::string base_dir = "../..";
std::string json_dir = "svg_element";
...
public:
    ...
    const std::string private_domain() const {
        std::stringstream ss;
        ss << "    private:" << std::endl;
        ss << "        double _" << dom_to_snake(_name) <<";" << std::endl;
        return ss.str();
    }
    const std::string init_default() const {
        std::stringstream ss;
        ss << "        _" << dom_to_snake(_name) <<" = NUM_NULL;" << std::endl;
        return ss.str();
    }
    const std::string getter_defination() const {
        ...
        ss << "        */" << std::endl;
        ss << "        double get_" << dom_to_snake(_name) << "() const;" << std::endl;
        return ss.str();
    }
    const std::string setter_defination() const {
        ...
        ss << "        *" << std::endl;
        ss << "        * @param " << dom_to_snake(_name) << " " <<  _comment << std::endl;
        ss << "        */" << std::endl;
        ss << "        void set_" << dom_to_snake(_name) << "(const double &" << dom_to_snake(_name) << ");" << std::endl;
        return ss.str();
    }
    const std::string getter_implemetation() const {
        std::stringstream ss;
        ss << "    double SVG" << dom_to_pascal(_tag) << "::get_" << dom_to_snake(_name) << "() const {" << std::endl;
    ...
    const std::string setter_implemetation() const {
        std::stringstream ss;
        ss << "    void SVG" << dom_to_pascal(_tag) << "::set_" << dom_to_snake(_name) << "(const double &" << dom_to_snake(_name) << ") {" << std::endl;
    ...
    const std::string str_get_attributes() const {
        std::stringstream ss;
        ss << "        if (_" << dom_to_snake(_name) << " != NUM_NULL) ss << \" " << _name << "=\\\"\" << _" << dom_to_snake(_name) << " << \"\\\"\";" << std::endl;;
        return ss.str();
    }
    const std::string str_operator_minus() const {
        std::stringstream ss;
        ss << "        if (_" << dom_to_snake(_name) << " != _element.get_" << dom_to_snake(_name) << "()) {" << std::endl;;
        ss << "            if (_" << dom_to_snake(_name) << " == NUM_NULL) ss << \"reset " << _name << "\" << std::endl;" << std::endl;;
...
public:
    ...
    const std::string private_domain() const {
        std::stringstream ss;
        ss << "    public:" << std::endl;
        ss << "        enum class " << dom_to_pascal(_name) <<" { ";
        for (int i = 0; i < _candidates.size(); i++) {
            ss << dom_to_snake(_candidates[i], true);
            if (i < _candidates.size() - 1) ss << ", ";
        }
        ss << " };" << std::endl;
        ss << "    private:" << std::endl;
        ss << "        " << dom_to_pascal(_name) <<" _" << dom_to_snake(_name) <<";" << std::endl;
        return ss.str();
    }
    const std::string init_default() const {
        std::stringstream ss;
        ss << "        _" << dom_to_snake(_name) <<" = " << dom_to_pascal(_name) <<"::" << dom_to_snake(_default, true) <<";" << std::endl;
        return ss.str();
    }
    const std::string getter_defination() const {
        std::stringstream ss;
        ...
        ss << "        * @return " << _comment << std::endl;
        ss << "        */" << std::endl;
        ss << "        const " << dom_to_pascal(_name) <<" get_" << dom_to_snake(_name) << "() const;" << std::endl;
        return ss.str();
    }
    const std::string setter_defination() const {
        std::stringstream ss;
        ...
        ss << "        * @param " << dom_to_snake(_name) << " " <<  _comment << std::endl;
        ss << "        */" << std::endl;
        ss << "        void set_" << dom_to_snake(_name) << "(const " << dom_to_pascal(_name) <<" &" << dom_to_snake(_name) << ");" << std::endl;
        return ss.str();
    }
    const std::string getter_implemetation() const {
        std::stringstream ss;
        ss << "    const SVG" << dom_to_pascal(_tag) << "::" << dom_to_pascal(_name) <<" SVG" << dom_to_pascal(_tag) << "::get_" << dom_to_snake(_name) << "() const {" << std::endl;
        ss << "        return _" << dom_to_snake(_name) << ";" << std::endl;
        ss << "    }" << std::endl;
        return ss.str();
    }
    const std::string setter_implemetation() const {
        std::stringstream ss;
        ss << "    void SVG" << dom_to_pascal(_tag) << "::set_" << dom_to_snake(_name) << "(const SVG" << dom_to_pascal(_tag) << "::" << dom_to_pascal(_name) <<" &" << dom_to_snake(_name) << ") {" << std::endl;
        ss << "        _" << dom_to_snake(_name) << " = " << dom_to_snake(_name) << ";" << std::endl;
        ss << "    }" << std::endl;
        return ss.str();
    }
    const std::string str_get_attributes() const {
        ...
        for (auto &c : _candidates) {
            if (c != _default) ss << "        if (_" << dom_to_snake(_name) << " == " << dom_to_pascal(_name) <<"::" << dom_to_snake(c, true) << ") ss << \" " << _name << "=\\\"" << c << "\\\"\";" << std::endl;
        }
        return ss.str();
    }
    const std::string str_operator_minus() const {
        std::stringstream ss;
        ss << "        if (_" << dom_to_snake(_name) << " != _element.get_" << dom_to_snake(_name) << "()) {" << std::endl;;
        ss << "            if (_" << dom_to_snake(_name) << " == " << dom_to_pascal(_name) <<"::" << dom_to_snake(_default, true) << ") ss << \"reset " << _name << "\" << std::endl;" << std::endl;;
        for (auto &c : _candidates) {
            if (c != _default) ss << "            else if (_" << dom_to_snake(_name) << " == " << dom_to_pascal(_name) <<"::" << dom_to_snake(c, true) << ") ss << \"modify " << _name << " \\\"" << c << "\\\"\" << std::endl;" << std::endl;
        }
        ss << "        }" << std::endl;
        return ss.str();
    }
