...
namespace Lewzen {
    VectorCoordinateSystem::VectorCoordinateSystem(const std::weak_ptr<Point2D> &A, const std::weak_ptr<Point2D> &B): _A(A), _B(B) {
    ...
    Point2D VectorCoordinateSystem::to_relative(const Point2D &p) const {
       if (p.get_coordinate_system() != *this) {
        ...
        if (sp1 && sp2) {
            ...
            return Point2D((l == 0) ? 0 : p.get_x() / l, (l == 0) ? 0 : p.get_y() / l, VectorRelativeCoordinateSystem(sp1, sp2));
        } else {
