...
std::string SVGCPP() {
    ...
    ss << "        _inner_text = STR_NULL;" << std::endl;
    ss << "" << std::endl;
    ss << "        _attribute_hash = 0;" << std::endl;
    ss << "        _inner_hash = 0;" << std::endl;
    ss << "        _outer_hash = 0;" << std::endl;
    ss << "    }" << std::endl;
    ss << "    SVGElement::SVGElement(const SVGElement &element) {" << std::endl;
    for (auto &p : attributes) ss << p->init_clone();
    ss << "" << std::endl;
    ss << "        _inner_text = element.get_inner_text();" << std::endl;
    ss << "        for (auto p : element.get_inner_elements()) {" << std::endl;
    ss << "            _inner_elements.push_back(p->clone());" << std::endl;
    ss << "        }" << std::endl;
    ss << "" << std::endl;
    ...
    ss << "        if (_inner_text != STR_NULL) ss << _inner_text << std::endl;" << std::endl;
    ss << "        for (int i = 0; i < _inner_elements.size(); i++) {" << std::endl;
    ss << "            if (i < _inner_elements.size() - 1) ss << _inner_elements[i]->outer_SVG() << std::endl;" << std::endl;
    ss << "            else ss << _inner_elements[i]->outer_SVG();" << std::endl;
    ss << "        }" << std::endl;
    ss << "        return ss.str();" << std::endl;
    ss << "    }" << std::endl;
    ss << "    void SVGElement::set_inner_text(const std::string &text) {" << std::endl;
    ss << "        _inner_text = text;" << std::endl;
    ss << "        update_inner_hash();" << std::endl;
    ss << "    }" << std::endl;
    ss << "    const std::string SVGElement::get_inner_text() const {" << std::endl;
    ss << "        return _inner_text;" << std::endl;
    ss << "    }" << std::endl;
    ss << "    void SVGElement::add_inner_element(const std::shared_ptr<SVGElement> &inner_element) {" << std::endl;
    ss << "        _inner_elements.push_back(inner_element);" << std::endl;
    ss << "        inner_element->_parent_elements.push_back(shared_from_this());" << std::endl;
    ss << "        update_inner_hash();" << std::endl;
    ss << "    }" << std::endl;
    ss << "    void SVGElement::remove_inner_element(const std::shared_ptr<SVGElement> &inner_element) {" << std::endl;
    ss << "        _inner_elements.erase(std::remove(_inner_elements.begin(), _inner_elements.end(), inner_element), _inner_elements.end());" << std::endl;
    ss << "        auto parents = inner_element->_parent_elements;" << std::endl;
    ss << "        parents.erase(std::remove_if(parents.begin(), parents.end(), [inner_element](auto &p){" << std::endl;
    ss << "            return !(p.owner_before(inner_element) || inner_element.owner_before(p));" << std::endl;
    ss << "        }), parents.end());" << std::endl;
    ss << "        update_inner_hash();" << std::endl;
    ss << "    }" << std::endl;
    ...
    ss << "            ss << \"/>\";" << std::endl;
    ss << "        } else {" << std::endl;
    ss << "            ss << \">\" << std::endl << _inner_svg << std::endl << \"</\" << get_tag() << \">\";" << std::endl;
    ss << "        }" << std::endl;
    ss << "" << std::endl;
    ss << "        return ss.str();" << std::endl;
    ss << "    }" << std::endl;
    ...
    ss << "        for (auto wp : _parent_elements) {" << std::endl;
    ss << "            if (auto sp = wp.lock()) sp->update_inner_hash();" << std::endl;
    ss << "        }" << std::endl;
    ss << "    }" << std::endl;
    ...
    ss << "        return std::make_shared<SVGElement>(*this);" << std::endl;
    ss << "    }" << std::endl;
    ...
    ss << "        if (element.get_attribute_hash() == get_attribute_hash()) return ss.str();" << std::endl;
    ss << "        ss << attribute_differ(_element);" << std::endl;
    ss << "" << std::endl;
    ss << "        // inner differ" << std::endl;
    ss << "        if (element.get_inner_hash() == get_inner_hash()) return ss.str();" << std::endl;
    ss << "        if (element.get_inner_text() != get_inner_text()) {" << std::endl;
    ss << "            auto content = get_inner_text();" << std::endl;
    ...
    ss << "        inner_differ(element, removal, addition, changed);" << std::endl;
    ss << "        // remove" << std::endl;
    ss << "        int n = element.get_inner_elements().size();" << std::endl;
    ss << "        int *removed = new int[n]," << std::endl;
    ss << "               *indices = new int[n];" << std::endl;
    ...
    ss << "            indices[changed.size() + i] = a.idx;" << std::endl;
    ss << "        }" << std::endl;
    ss << "        ss << \"sort \\\"\";" << std::endl;
    ss << "        for (int i = 0; i < n; i++) {" << std::endl;
    ss << "            ss << indices[i];" << std::endl;
    ss << "            if (i < n - 1) ss << \", \";" << std::endl;
    ss << "        }" << std::endl;
    ...
    ss << "        return ss.str();" << std::endl;
    ss << "    }" << std::endl;
    ss << "    const std::string SVGElement::inner_differ(const SVGElement &element," << std::endl;
    ss << "            std::vector<_el_idx> &removal," << std::endl;
    ss << "            std::vector<_el_idx> &addition," << std::endl;
    ...
    ss << "        for (auto &b : element.get_inner_elements()) tags_map[b->get_tag()].insert({b, c}), B.insert({b, c++});" << std::endl;
    ss << "" << std::endl;
    ss << "        c = 0;" << std::endl;
...
std::string SVGH(const std::vector<std::string> &tags) {
    ...
    ss << "        /**" << std::endl;
    ss << "        * Deep copy of SVG element." << std::endl;
    ss << "        */" << std::endl;
    ss << "        SVGElement(const SVGElement &element);" << std::endl;
    ss << "" << std::endl;
    ss << "        /// Tag" << std::endl;
    ss << "    public:" << std::endl;
    ss << "        /**" << std::endl;
    ...
    ss << "    private:" << std::endl;
    ss << "        std::string _inner_text;" << std::endl;
    ss << "        std::vector<std::shared_ptr<SVGElement>> _inner_elements;" << std::endl;
    ss << "        std::vector<std::weak_ptr<SVGElement>> _parent_elements;" << std::endl;
    ss << "    protected:" << std::endl;
    ...
    ss << "        * @return DOM Commands." << std::endl;
    ss << "        */" << std::endl;
    ss << "        const std::string inner_differ(const SVGElement &element," << std::endl;
    ss << "            std::vector<_el_idx> &removal," << std::endl;
    ss << "            std::vector<_el_idx> &addition," << std::endl;
    ss << "            std::vector<std::pair<_el_idx, _el_idx>> &changed) const;" << std::endl;
    ss << "    public:" << std::endl;
    ...
    ss << "        */" << std::endl;
    ss << "        void add_inner_element(const std::shared_ptr<SVGElement> &element);" << std::endl;
    ss << "        /**" << std::endl;
    ss << "        * Remove a sub element from this SVG element." << std::endl;
    ss << "        *" << std::endl;
    ss << "        * @param element an inner element." << std::endl;
    ss << "        */" << std::endl;
    ss << "        void remove_inner_element(const std::shared_ptr<SVGElement> &element);" << std::endl;
    ss << "        /**" << std::endl;
